...
function getMergedCourses(day) {
    ...
    courses.forEach((course) => {
        ...
        const startPeriodIndex = periods.findIndex((p) => timeToMinutes(p.time.split(" - ")[0]) === courseStart);
        ...
        if (!mergedCourses.some((c) => c.startPeriodIndex <= startPeriodIndex && c.endPeriodIndex > startPeriodIndex)) {
            mergedCourses.push({
                ...course,
                startPeriodIndex,
                endPeriodIndex,
                            ...
                            {["Thứ 2", "Thứ 3", "Thứ 4", "Thứ 5", "Thứ 6", "Thứ 7"].map((day) => {
                                ...
                                if (course) {
                                    return (
                                        ...
                                        >
                                            <strong className={cx("course-code")}>IE101.P22 - VN</strong>
                                            <br />
                                            <span className={cx("course-title")}>{course.subject}</span>
                                            <br />
                                            <strong className={cx("teacher-name")}>{course.teacher}</strong>
                                            <br />
                                            <span className={cx("room")}>{course.room}</span>
                                            <br />
                                            {course.start && course.end && (
                                                <span className={cx("course-date")}>
                                                    BD: {course.start} <br /> KT: {course.end}
                                                </span>
                                            )}
